...
class DynamicReLU_B(nn.Module):
    ...
    def forward(self, x):
        ...
        y = self.avg_pool(x).view(b, c)
        z = self.dynamic(y)
        zz=z.view(-1, self.channels, 2 * self.K).cuda()
        relu_coefs = (zz * self.lambdas + self.init_v)
        x_perm = x.permute(2, 3, 0, 1).unsqueeze(-1).to(device="cuda")
        output = (x_perm * relu_coefs[:, :, :self.K] + relu_coefs[:, :, self.K:]).to(device="cuda")
        output = torch.max(output, dim=-1)[0].permute(2, 3, 0, 1).to(device="cuda")
        return output
